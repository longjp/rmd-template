...
p <- ggsurvplot(fit,data=mgus,risk.table=TRUE,ylim=c(0,1),xlim=c(0,5000),
                ...
                legend.title="Gender",legend.labs=levels(mgus$sex))
p$plot <- p$plot + theme_grey() + theme(legend.position = c(0.8, 0.8))
print(p)

??ggsurvplot

## see if we can do example with pval=TRUE
```
...
Output pretty contingency tables in kable format:
```{r table-table}
library(kableExtra)
library(tidyverse)
...
vvar <- c("M1","M2")
vname <- c("Model 1","Model 2")

tab <- table(dat[,vvar[1]],dat[,vvar[2]])
tab
tab <- as.data.frame(tab)
colnames(tab) <- c(vname,"n")
tab <- tibble(tab)
tab <- tab %>% pivot_wider(names_from = vname[2], values_from = n)
tab
tab <- tab %>% 
    rename("\t" = vname[1]) %>% 
    add_column(" " = c(vname[1], rep(" ", nrow(.) - 1)), .before = "\t") %>% 
    kable() %>% 
    kable_styling(position = "left", full_width = F, ) %>% 
    add_header_above(c("","","",vname[2], rep(" ", ncol(tab) - 3))) %>% 
...
Using the code above and flextable, we can output the gtsummary table to word. To reduce size of cells in word, right click on tables, table properties, row, and then specify 0.25in exact height.

```{r word-table}
library(flextable)
tab <- as_flex_table(add_p_ex1)
save_as_docx(tab,path="table1.docx")
```
